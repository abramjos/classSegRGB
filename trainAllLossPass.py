...
def train_epochAll(epoch, Dataloader, Criteria, Model, Optimizer, device = 'cpu', threshold = 0.5):
  ...
  for idx,datax in enumerate(Dataloader):
    ...
    for i in ['Encoder','Decoder','Classifier','Segmentation']:#Model.keys():
      ...
      if i  in 'Decoder':
        ...
        X_hat,X_hatC,X_hatS = x_hat.clone().detach(),x_hatC.clone().detach(),x_hatS.clone().detach()
